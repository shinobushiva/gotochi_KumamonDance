 ...
 * 
 */
public abstract class PrefecturesActivityBase extends UnityPlayerActivity {
	...
	/**
	 * 他都道府県に移動したときに発行されるインテントに付属するアクション. このアクションを含むインテントはご当地サービスによって発行される.
	 */
	...
	/**
	 * ご当地アプリで利用できるサービスのインスタンスを取得する. <br/> {@link #onServiceBind() onServiceBind}
	 * 後でなければインスタンスを取得することはできない. <br/> {@link #isGotochiApp() isGotochiApp}
	 * がfalseなら、このメソッドは常にnullを返す.
	 * 
	...
	private boolean checkGotochiApp() {
		...
		if (pm.checkPermission(Manifest.permission.GET_TASKS, this
		...
		try {
			pm.getServiceInfo(new ComponentName(GOTOCHI_SERVICE_PACKAGE_NAME,
					GOTOCHI_SERVICE_CLASS_NAME), 0);
	...
	private boolean isFinishSelfApps() {
		ActivityManager am = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
		try {
			List<RunningTaskInfo> infoList = am
					.getRunningTasks(Integer.MAX_VALUE);
			int size = infoList.size();
			for (int i = 0; i < size; ++i) {
				RunningTaskInfo info = infoList.get(i);
				Bundle metaData = getPackageManager().getActivityInfo(
						info.baseActivity, PackageManager.GET_META_DATA).metaData;

				if (metaData != null
						&& metaData.getBoolean(
								PrefecturesActivityBase.METADATA_GOTOCHI_APP,
								false) && info.numRunning != 0)
					return false;
			}
		} catch (SecurityException e) {
			// 事前にパーミッションのチェックをしているが一応例外ハンドラを書いておく
	...
	/**
	 * ご当地サービスのインスタンスを取得したときに実行される. <br/>
	 * サービスインスタンスから何らかのデータを取得したい場合は このメソッドをオーバーライドしその中で記述するか
	 * このメソッドの実行後でなければならない. <br/>
	 * メソッドをオーバーライドする時は 必ず先頭でsuperクラスのonServiceBindを実行しなければならない.
	 * 
	 * @param service
	 *            ご当地サービスのインスタンス
	 */
	...
	/**
	 * 都道府県を移動したときに実行される. <br/>
	 * メソッドをオーバーライドしtrueを戻り値にした場合 Activityは終了しない. <br/>
	 * オーバーライドしない、もしくはfalseを戻り値にした場合 Activityは強制的に終了する.
	 * 
	 * @param context
	 *            {@link BroadcastReceiver#onReceive(Context, Intent)}のContext引数
	 * @param intent
	 *            {@link BroadcastReceiver#onReceive(Context, Intent)}のIntent引数
